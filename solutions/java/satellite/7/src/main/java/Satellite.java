...
public class Satellite {
    // 1_000 is just a random value, to get a meaningful value I should profile the code
    ...
    private static final <T extends Comparable<? super T>> int binarySearch(T[] arr, int low, int high, T key) {
        for (int m = (low + high) >> 1; low < high; m = (low + high) >> 1) {
            T val = arr[m];
    ...
    private static final Node buildRecursively(Iterator<Character> iter, Character[] inorder, int low, int high) {
        ...
        int rootValIndex = binarySearch(inorder, low, high, rootVal);
