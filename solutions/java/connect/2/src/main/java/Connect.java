...
public class Connect {
    ...
    private boolean shouldCashResult = false;
    private Optional<Winner> cachedResult = Optional.empty();
    ...
    public Connect(String []board, boolean shouldCashResult) {
        this.board = board;
        this.shouldCashResult = shouldCashResult;
    }
    ...
    public Winner computeWinner() {
        ...
        if (cachedResult.isEmpty()) {
            cachedResult = Optional.of(compute());
        }
        return cachedResult.get();
    }

    private Winner compute() {
        if (board.length == 0) {
            return Winner.NONE;
        }
