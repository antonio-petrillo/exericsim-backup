import java.util.Collection;
import java.util.Iterator;
import java.util.function.Function;
import java.util.function.BiFunction;
import java.util.function.Predicate;
import java.util.function.Consumer;

public class CustomSet<T> implements Iterable<T> {
    private static final int DEFAULT = 16;
    private static final float THRESHOLD = 0.75f;
    private static final float DEL_THRESHOLD = 0.10f;
    private static final int BIG_PRIME = 115249;
    private static final Object TOMBSTONE = new Object();
    
    private Object[] container = new Object[DEFAULT];
    private int cap = 0;
    
    public CustomSet() { }
    
    public CustomSet(Collection<T> data) {
        for (var elem : data) {
            add(elem);
        }
    }
    
    private CustomSet(CustomSet<T> set) {
        container = set.container.clone();
        cap = set.cap;
    }
    
    @SuppressWarnings("unchecked")
    private void growth() {
        var old = container;
        container = new Object[container.length << 1];
        cap = 0;
        for (var o : old) {
            if (o != null && o != TOMBSTONE) {
                add((T) o);
            } 
        }
    }
     
    @SuppressWarnings("unchecked")
    private void shrink() {
        var old = container;
        container = new Object[container.length >> 1];
        cap = 0;
        for (var o : old) {
            if (o != null && o != TOMBSTONE) {
                add((T) o);    
            }
        }
    }
    
    public int size() {
        return cap;
    }
    
    private static int getIndex(Object o, int i, int size) {
        return Math.floorMod(BIG_PRIME * i ^ o.hashCode(), size);
    } 
    
    public boolean add(T element) {
        for(int i = 0; i < container.length; i++) {
            var index = getIndex(element, i, container.length);
            if (container[index] == null || container[index] == TOMBSTONE) {
                container[index] = element;
                break;
            }
            if (element.equals(container[index])) {
                return false; // elem already present
            }
        }
        cap++;
        if (cap / (double) container.length >= THRESHOLD) {
            growth();
        }
        return true;
    }
    
    public boolean remove(Object o) {
        int oldCap = cap;
        for(int i = 0; i < container.length; i++) {
            var index = getIndex(o, i, container.length);
            if (container[index] != null && container[index].equals(o)) {
                container[index] = TOMBSTONE;
                cap--;
                if (cap / (double) container.length <= DEL_THRESHOLD && container.length > DEFAULT) {
                    shrink();
                }
                break;
            }
        }
        return cap < oldCap;
    }
    public Iterator<T> iterator() {
        return new Iterator<T>() {
            int found = 0, index = 0;
            
            @Override
            public boolean hasNext() {
                return found < cap;
            }
            @SuppressWarnings("unchecked")
            @Override
            public T next() {
                while(true) {
                    if (container[index] != null) {
                        found++;
                        T next = (T)container[index];
                        index++;
                        return next;
                    }
                    index++;
                }
            }
        };
    }
    
    public boolean isEmpty() {
        return cap == 0;
    }
    
    public boolean contains(Object target) {
        for(var element : this) {
            if(element.equals(target)) {
                return true;
            }
        }
        return false;
    }
    
    public boolean isDisjoint(CustomSet<?> other) {
        for (var elem : other) {
            if (contains(elem)) {
                return false;
            }
        }
        return true;
    }
    
    @Override
    public boolean equals(Object obj) {
        if (obj == null) {
            return false;
        }
        if (obj == this) {
            return true;
        }
        if (!(obj instanceof CustomSet<?>)) {
            return false;
        }
        CustomSet<?> other = (CustomSet<?>) obj;
        if (cap != other.cap) {
            return false;
        }
        for (var elem : other) {
            if (!contains(elem)) {
                return false;
            }
        }
        return true;
    }
    
    public CustomSet<T> getIntersection(CustomSet<?> other) {
        CustomSet<T> intersection = new CustomSet<>();
        for (var elem : this) {
            if (other.contains(elem)) {
                intersection.add(elem);
            }
        }
        return intersection;
    }
    
    public CustomSet<T> getUnion(CustomSet<? extends T> other) {
        CustomSet<T> union = new CustomSet<>(this);
        for (T elem : other) {
            union.add(elem);
        }
        return union;
    }
    
    public CustomSet<T> getDifference(CustomSet<? extends T> other) {
        CustomSet<T> difference = new CustomSet<>();
        for (T elem : this) {
            if (!other.contains(elem)) {
                difference.add(elem);    
            }
        }
        return difference;
    }
    
    public boolean isSubset(CustomSet<?> other) {
        for(var elem : other) {
            if(!contains(elem)) {
                return false;
            }
        }
        return true;
    }

    // Can't generalized the type of other
    public CustomSet<T> getSymmetricalDifference(CustomSet<T> other) {
        CustomSet<T> diff = new CustomSet<>();
        for (var elem : this) {
            if (!other.contains(elem)) {
                diff.add(elem);
            }
        }
        for (var elem : other) {
            if (!contains(elem)) {
                diff.add(elem);
            }
        }
        return diff;
    }

    public <U> CustomSet<U> map(Function<? super T, ? extends U> f) {
        CustomSet<U> mapped = new CustomSet<>();
        for (var elem : this) {
            mapped.add(f.apply(elem));
        }
        return mapped;
    }

    public CustomSet<T> filter(Predicate<? super T> p) {
        CustomSet<T> filtered = new CustomSet<>();
        for (var elem : this) {
            if(p.test(elem)) {
                filtered.add(elem);    
            }
        }
        return filtered;
    }

    public <U> U reduce(BiFunction<? super T, ? super U, ? extends U> f, U initial) {
        for (var elem : this) {
            initial = f.apply(elem, initial);
        }
        return initial;
    }

    public void forEach(Consumer<? super T> f) {
        for (var elem : this) {
            f.accept(elem);
        }
    }
}
